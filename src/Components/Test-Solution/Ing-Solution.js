...
function GameInterface() {
  ...
  const [correctSolutions, setCorrectSolutions] = (null);
  ...
  const fetchCorrectSolution = async (levelId) => {
    try {
      const response = await axios.get(`/api/solution/${levelId}`);
      setCorrectSolution(response.data);
    } catch (error) {
  ...
  const validateSolution = () => {
    if(!correctSolutions || correctSolutions.length === 0) return;
    let highestScore = 0;

    correctSolutions.for
  }

  useEffect(() => {
    fetchCorrectSolution(1);
  })
